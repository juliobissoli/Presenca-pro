...
import { useGetClasses, useCreateClass, useUpdateClass, useDeleteClass } from "../hooks/classes";
...
export default function HomePage({ user, onLogin, onLogout }: { user: User | null, onLogin: (u: User) => void, onLogout: () => void }) {
    ...
    <Layout user={user} onLogout={onLogout}>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setIsCreateClassOpen(true)} size="sm" className="gap-2">
        ...
        <div className="grid gap-4">
          {isLoading ? (
            <div className="text-center py-12">
              <p className="text-muted-foreground animate-pulse">Carregando turmas...</p>
            </div>
          ) : (
            <>
              {data?.data.map((cls) => (
                ...
                >
                  <CardContent className="p-6 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{cls.studentCount} alunos</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={(e) => handleEditClass(e, cls)}>
              ...
              {(data?.data.length === 0) && (
                <div className="text-center py-12 border-2 border-dashed rounded-lg">
                  ...
                  <p className="mt-4 text-muted-foreground">Nenhuma turma cadastrada</p>
                </div>
              )}
            </>
          )}
      ...
      <Dialog open={isCreateClassOpen} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editingClass ? "Altere o nome da turma abaixo." : "Insira o nome da nova turma para começar."}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsCreateClassOpen(false)}>Cancelar</Button>
            <Button onClick={handleCreateClass} disabled={isCreating || isUpdating}>
              {isCreating || isUpdating ? "Salvando..." : editingClass ? "Salvar" : "Criar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
